...
packages = f"any-snapshot-hierarchical linux-image-{ARCH} btrfs-progs curl sudo dhcpcd network-manager locales nano console-setup \
...
def main():
    ...
    #   Go inside chroot
    os.system("cp /root/any-snapshot-hierarchical.deb /mnt/var/cache/apt/archives/")
...
def strap():
    ...
    sp.check_call(f"debootstrap --arch={ARCH} {RELEASE} /mnt https://deb.debian.org/debian", shell=True) # REVIEW --include={packages} ? --variant=minbase ? --exclude={excl}
